...
import { adminGetQuotation } from "@/lib/admin.quotations.functions";
import { Button } from "@/components/ui/button";
import { ArrowLeft, Printer, Mail, MessageCircle, Download } from "lucide-react";

export const Route = createFileRoute("/_authenticated/salesman/quotations/$id")({
  head: () => ({ meta: [{ title: "Salesman · Quotation" }] }),
...
function QuotationDetail() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["salesman-quotation", id],
    queryFn: () => get({ data: { id } }),
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-3">
          <Link to="/salesman/quotations"><Button variant="outline" size="sm"><ArrowLeft className="h-4 w-4" /></Button></Link>
      ...
      <div className="rounded-lg border bg-surface p-3 flex flex-wrap gap-2">
        {quote.converted_order_id && (
          <Link to="/salesman/orders/$id" params={{ id: quote.converted_order_id }}>
